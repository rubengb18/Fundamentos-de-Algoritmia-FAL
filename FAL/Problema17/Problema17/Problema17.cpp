...
void resolver(vector<int>v, int ini, int fin,int cant,bool &found,int &res) {
	int mit = (fin + ini) / 2;
	if (mit + cant == v[mit]) {
		found = true;
		res = v[mit];
		...
		if (mit + cant < v[mit]) {
			resolver(v, ini, mit, cant, found, res);
		}
		else {
			resolver(v, mit + 1, fin, cant, found, res);
		}
	}
}
...
void resuelveCaso() {
	...
	resolver(v, 0, v.size() - 1, cant, found,res);
	if (found) {
		cout << res << '\n';
	}
	else {
